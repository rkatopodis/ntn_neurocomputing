# Taken directly from Open AI Baselines:
...
def evaluation_curve(evaluation, radius=None):
    evaluation = np.load(evaluation) if isinstance(evaluation, str) else evaluation

    x, y = evaluation["timesteps"], np.mean(evaluation["results"], axis=1)
  
    y = smooth(y, radius=radius) if radius is not None else y

    return (x, y)

def trial_curve(resultsdir, radius=None, resample=512, smooth_step=1.0, shaded_std=True, shaded_err=True):
    evals = glob(f"{Path(resultsdir)}/*/*.npz")
    xys = list(map(partial(evaluation_curve, radius=radius), evals))

    origxs = list(map(lambda t: t[0], xys)) # [xy[0] for xy in xys]
    minxlen = min(map(len, origxs))
    def allequal(qs):
        return all((q==qs[0]).all() for q in qs[1:])
    if resample:
    ...
    ax.plot(usex, ymean)
    if shaded_err:
        ax.fill_between(usex, ymean - ystderr, ymean + ystderr, alpha=.4)
    if shaded_std:
        ax.fill_between(usex, ymean - ystd,    ymean + ystd,    alpha=.2)
